...
MbedClient::MbedClient(): _led(LED1, 1), _relay(PTB9, 0)
{
    _interface = NULL;
...
bool MbedClient::create_interface()
{
   ...
   const char *rf_type = rf_board_type();
    ...
    char *mac = trace_array(mac_addr, 8);
        ...
        String info_type = "CH-";
        info_type.append(STR(YOTTA_CFG_MBED_MESH_API_6LOWPAN_ND_CHANNEL), strlen(STR(YOTTA_CFG_MBED_MESH_API_6LOWPAN_ND_CHANNEL)));
        info_type.append("-", 1);
        info_type.append(rf_type, strlen(rf_type));
        info_type.append(mac, strlen(mac));
    #else
        String info_type = "RF-Type";
        info_type.append(rf_type, strlen(rf_type));
        info_type.append("-", 1);
        info_type.append(mac, strlen(mac));
    #endif
...
M2MObject *MbedClient::create_generic_object()
{
    ...
    if (_object) {
        ...
        if (inst) {
            M2MResource *res = inst->create_dynamic_resource("E",
                               "LightControl",
...
void MbedClient::update_resource()
{
    if (_object) {
        ...
        if (inst) {
            M2MResource *res = inst->resource("E");
            char buffer[20];
            int size = sprintf(buffer, "%d", _value);
            if(res) {
                res->set_value((const uint8_t *)buffer,
...
void MbedClient::value_updated(M2MBase *base, M2MBase::BaseType type) {
    ...
    M2MResource *light_resource = object_instance->resource("E");
    ...
    int val = atoi((const char *) tmp_ptr);
